...
DOES THIS INCLUDE ANIMATION? (add animation to the input widget so when the user presses play the input widget scrolls through the range automatically.)
```{r}
var_choices <- names(df)[3:7]
...
df_nomiss <- df_major %>% filter(!is.na(discipline)) %>%
  mutate(discipline = factor(discipline)) %>% 
  filter(discipline == "MATH" | 
           discipline == "STAT" |
           discipline == "CS")

#why including all the disciplines
majors <- levels(df_nomiss$discipline) 

ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(radioButtons("majorchoice", label = "Choose a Major",
                             choices = df_nomiss$discipline)),
    mainPanel(plotOutput("m_plot"))
  )
)
...
server <- function(input, output, session) {
  ...
  major_reactive_df <- reactive({
    ...
    df_choice <- df_nomiss %>% filter(discipline == input$majorchoice) 
  df_full <- semi_join(df_long, df_choice, by = "adm_id") %>%
    filter(type == "major1" |
             type == "major2" | 
             type == "major3")
  
  df_nochoice <- df_full %>% filter(discipline != input$majorchoice &
                                        !is.na(discipline)) %>%
    group_by(discipline) %>%
    summarise(nstudent = n()) %>%
    mutate(discipline = fct_reorder(discipline, nstudent))
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(selectInput("majorchoice", label = "Choose a Major",
                             choices = majors)),
    mainPanel(plotOutput("m_plot"), tableOutput("m_table"))
  )
)
...
sex_reactive_df <- reactive({
  ...
  df_nostat <- df_statfull %>% filter(discipline != input$majorchoice &
    ...
    filter(sex == "F") %>%
    group_by(discipline)
    summarise(fems = n(), males = nstudent - fems)
  })

  output$m_table <- renderTable({
    head(sex_reactive_df()) #WHY THIS TABLE NOT WORK
  })
}
...
```{r}
library(tidyverse)
stat <- read_csv("data/stat113_survey.csv") %>% filter(!is.na(Facebook), !is.na(Year)) %>% filter(GPA <= 4)
```

(7 points). Construct a graph that uses the number of Facebook friends each student has. CHANGE OVER TIME
```{r}
ggplot(stat, aes(x = factor(time_year), y = Facebook)) +
  geom_boxplot() #fct_inorder() = graphs in order of column in df

fb_mean <- stat %>% group_by(time_both) %>% filter(!is.na(Facebook)) %>%
  summarise(mean = mean(Facebook, na.rm = TRUE),
            nstudent = n(),
            sdfacebook = sd(Facebook, na.rm = TRUE)) %>%
  mutate(se = sdfacebook / sqrt(nstudent), se_1 = mean - se,
         se_u = mean + se) %>%
  mutate(time_both = fct_drop(time_both))

ggplot(stat %>% filter(!is.na(Facebook)),
       aes(x = time_both, y = Facebook)) +
  geom_point(alpha = .1) +
  geom_point(data = fb_mean, aes(x = time_both, y = mean), color = "red") +
  geom_errorbar(data = fb_mean, aes(y=mean, ymin = se_1, ymax = se_u))
```
...
stat_face <- stat %>% 
  ...
  mutate(Face = n()) %>%
  ungroup() %>%
  summarize(time_year = time_year, prop = Face/tot) 

ggplot(stat_face, aes(x = time_year, y = prop)) +
  geom_col()
```
...
ggplot(hpi_df, aes(x = Footprint)) +
  geom_freqpoly(aes(color = Region), bins = 10) +
...
An app to look at the vaarious sports and the relation to age and variables of money making. This includes salary, total pay, and endorsements.
...
```{r}
library(tidyverse)
athlete <- read_csv("data/athletesdata.csv") 
  
athlete_test <- athlete %>% filter(Sport == "Baseball")

ggplot(data = athlete_test, 
       aes(x = age, y = salary)) +
  geom_point()
```
...
```{r}
var_choices <- names(athlete)[5:7]

library(shiny)

ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(selectInput("sportchoice", 
                             label = "Choose a Sport",
                             choices = levels(factor(athlete$Sport))),
                 radioButtons("varchoice", label = "Choose a Statistic",
                 choices = var_choices)),
...
server <- function(input, output, session) {
  athlete_test <- reactive({
    athlete %>% filter(Sport == input$sportchoice)
  })
  
  plot_test <- reactive({
    ggplot(data = athlete_test(), 
       aes(x = age, y = .data[[input$varchoice]])) +
  geom_point()
  })
...
If we wanted a bar graph with each discipline and the number of students majoring in that discipline -- the shiny app allowing us to choose the sex of the student. Nothing would be changing in terms of the graph, as the input value (majors) and the measurement (num of students) remains constant.

OR if we wanted to show minors associated with a specific major (cuz no gaurantee that a user would select that major)
...
ui <- fluidPage(
  sidebarLayout(
    sidebarPanel(selectizeInput("playerchoice",
                 ...
                 selectizeInput("playerchoice2",
                                label = "Choose a Player", 
                                choices = levels(factor(df$player)),
                                selected = "Alex De Minaur"),
                 radioButtons("varchoice", 
                              label = "Choose a Statistic",
                              choices = var_choices),
                 radioButtons("meanchoice", label = "Display the Mean?", choices = c("YES", "NO"))),
    mainPanel(plotOutput("histgraph"), plotOutput("sidebyside"))
  )
)
...
server <- function(input, output, session) {
  ...
  sidebyside_plot <- reactive({
    # ggplot(df_sub(), aes_string(x = input$varchoice)) +
    # geom_histogram(colour = "black", fill = "white", bins = 15)
    ggplot(df_sub2(), aes(x = player, y = svperc)) +
  geom_boxplot() 
  })
  
  sidebyside_plot_mean <- reactive({
    # ggplot(df_sub(), aes_string(x = input$varchoice)) +
    # geom_histogram(colour = "black", fill = "white", bins = 15)
    ggplot(df_sub2(), aes(x = player, y = svperc)) +
  geom_boxplot() +
  ...
  output$sidebyside <- renderPlot({
    if(input$meanchoice == "NO") {
    sidebyside_plot()
    }
    else {
      sidebyside_plot_mean()
    }
  })
}
...
test
```{r}
df_sub2 <- df %>% filter(player == "Aryna Sabalenka" | player == "Alex De Minaur")
...
10 Reactivity and Other Topics in shiny
